...
def get_case_status(case_type, case_no, case_year):
...
def case_list(payload, cookies):
    ...
    for row in rows:
        ...
        if len(cols) > 0:
            ...
            if cols[0] is not None and cols[0].text.strip() == "No Data Found":
                break
            elif len(links) > 0:
...
def case_details(payload, cookies):
    ...
    data = {'petitioner': html.find(id="ctl00_content2_lblpartyname3").text.strip(),
            'respondent': html.find(id="ctl00_content2_lblpartyname1").text.strip()}
...
def inputs():
    input_list = [raw_input("Enter Case Type ??"), raw_input("Enter Case No ??"), raw_input("Enter Case Year ??")]
...
if __name__ == '__main__':
    input_list = inputs()
    lists = get_case_status(input_list[0], input_list[1], input_list[2])
    if not lists:
